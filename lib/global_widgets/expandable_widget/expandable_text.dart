...
import 'expandable_widgets.dart';
...
class ExpandableText extends StatefulWidget {
  ...
  const ExpandableText({
    ...
    this.boxShadow,
  })  : assert(helperTextList.length == 2,
...
class _ExpandableTextState extends State<ExpandableText>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      setState(() => _shouldShowHelper =
          widget.textWidget.hasOverflow(context.size?.width ?? 0.0));
    });
  }
  ...
  AnimatedCrossFade _bodyWithText() {
    TextStyle _defaultHelperStyle = const TextStyle(
        color: Colors.blue,
      ...
      secondChild: widget.textWidget.toRichText(
        TextSpan(
          ...
          style: widget.helperTextStyle
                  ?.copyWith(backgroundColor: widget.backgroundColor) ??
              _defaultHelperStyle,
  ...
  Flex _getChildWithArrow(ArrowLocation locationParameter, int? maxLineOption) {
    Widget _child = widget.textWidget.copyWith(
        maxLines: _isExpanded ? maxLineOption : widget.textWidget.maxLines);
